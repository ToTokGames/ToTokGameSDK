...
#error TTGCProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "TTGCProgressHUD.h"
#import "TTGCIndefiniteAnimatedView.h"
#import "TTGCProgressAnimatedView.h"
#import "TTGCRadialGradientLayer.h"

NSString * const TTGCProgressHUDDidReceiveTouchEventNotification = @"TTGCProgressHUDDidReceiveTouchEventNotification";
NSString * const TTGCProgressHUDDidTouchDownInsideNotification = @"TTGCProgressHUDDidTouchDownInsideNotification";
NSString * const TTGCProgressHUDWillDisappearNotification = @"TTGCProgressHUDWillDisappearNotification";
NSString * const TTGCProgressHUDDidDisappearNotification = @"TTGCProgressHUDDidDisappearNotification";
NSString * const TTGCProgressHUDWillAppearNotification = @"TTGCProgressHUDWillAppearNotification";
NSString * const TTGCProgressHUDDidAppearNotification = @"TTGCProgressHUDDidAppearNotification";

NSString * const TTGCProgressHUDStatusUserInfoKey = @"TTGCProgressHUDStatusUserInfoKey";

static const CGFloat TTGCProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat TTGCProgressHUDUndefinedProgress = -1;
static const CGFloat TTGCProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat TTGCProgressHUDVerticalSpacing = 12.0f;
static const CGFloat TTGCProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat TTGCProgressHUDLabelSpacing = 8.0f;


@interface TTGCProgressHUD ()
...
@property (nonatomic, strong) TTGCRadialGradientLayer *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) TTGCProgressAnimatedView *ringView;
@property (nonatomic, strong) TTGCProgressAnimatedView *backgroundRingView;
...
@implementation TTGCProgressHUD {
    BOOL _isInitializing;
}

+ (TTGCProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static TTGCProgressHUD *sharedView;
#if !defined(TTGC_APP_EXTENSIONS)
...
+ (void)setDefaultStyle:(TTGCProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(TTGCProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(TTGCProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:TTGCProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:TTGCProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(TTGCProgressHUDMaskType)maskType {
    TTGCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)dismissWithCompletion:(TTGCProgressHUDDismissCompletion)completion {
...
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(TTGCProgressHUDDismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = TTGCProgressHUDMaskTypeNone;
        _defaultStyle = TTGCProgressHUDStyleLight;
        _defaultAnimationType = TTGCProgressHUDAnimationTypeFlat;
        ...
        NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"TTGCProgressHUD" ofType:@"bundle"];
        NSBundle *imageBundle = [NSBundle bundleWithPath:bundlePath];
        
//        NSBundle *bundle = [NSBundle bundleForClass:[TTGCProgressHUD class]];
//        NSURL *url = [bundle URLForResource:@"TTGCProgressHUD" withExtension:@"bundle"];
//        NSBundle *imageBundle = [NSBundle bundleWithURL:url];
        ...
        _fadeInAnimationDuration = TTGCProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = TTGCProgressHUDDefaultAnimationDuration;
        ...
        self.accessibilityIdentifier = @"TTGCProgressHUD";
...
- (void)updateHUDFrame {
    ...
    hudWidth = TTGCProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + TTGCProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = TTGCProgressHUDVerticalSpacing + labelHeight + contentHeight + TTGCProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += TTGCProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(TTGCProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - TTGCProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != TTGCProgressHUDUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + TTGCProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-TTGCProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(TTGCProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-TTGCProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(TTGCProgressHUDParallaxDepthPoints);
...
#else
            // If TTGCProgressHUD is used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{TTGCProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
#if !defined(TTGC_APP_EXTENSIONS) && TARGET_OS_IOS
...
#elif !defined(TTGC_APP_EXTENSIONS) && !TARGET_OS_IOS
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:TTGCProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:TTGCProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak TTGCProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong TTGCProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = TTGCProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != TTGCProgressHUDMaskTypeNone) {
    ...
    if(self.backgroundView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:TTGCProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.backgroundView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:TTGCProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(TTGCProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:TTGCProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(self.backgroundView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:TTGCProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == TTGCProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[TTGCIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[TTGCIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        TTGCIndefiniteAnimatedView *indefiniteAnimatedView = (TTGCIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (TTGCProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[TTGCProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (TTGCProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[TTGCProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == TTGCProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == TTGCProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == TTGCProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [TTGCRadialGradientLayer layer];
        }
        ...
        if(self.defaultMaskType == TTGCProgressHUDMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == TTGCProgressHUDMaskTypeCustom){
...
- (void)fadeInEffects {
    if(self.defaultStyle != TTGCProgressHUDStyleCustom) {
        // Add blur effect
        UIBlurEffectStyle blurEffectStyle = self.defaultStyle == TTGCProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)setDefaultAnimationType:(TTGCProgressHUDAnimationType)animationType {
